...
from dakotaContigFilter.dakotaContigFilterImpl import dakotaContigFilter
from dakotaContigFilter.dakotaContigFilterServer import MethodContext
from dakotaContigFilter.authclient import KBaseAuth as _KBaseAuth

from installed_clients.AssemblyUtilClient import AssemblyUtil
from installed_clients.WorkspaceClient import Workspace


class dakotaContigFilterTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        for nameval in config.items('dakotaContigFilter'):
        ...
        cls.serviceImpl = dakotaContigFilter(cls.cfg)
    ...
    def test_run_dakotaContigFilter_min_len_negative(self):
        with self.assertRaisesRegex(ValueError, 'min_length parameter cannot be negative'):
            self.serviceImpl.run_dakotaContigFilter(self.ctx,
                                              {'workspace_name': self.wsName,
                                               'assembly_input_ref': '1/fake/3',
                                               'min_length': '-10'})

    def test_run_dakotaContigFilter_min_len_parse(self):
